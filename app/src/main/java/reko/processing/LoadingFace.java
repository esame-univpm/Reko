...
/**
 * The LoadingFace class is used to load the result of a detect face request.
 */
public class LoadingFace extends MainView{

    private byte[] imageBytes;

    private int setLoadingBar = 1;

    public LoadingFace(ViewsController viewsController) {
    ...
    public void setImage(String pathSelectedImage) {
        if(pathSelectedImage!=null){
            ...
            ((FaceResult) viewsController.getViewsMap().get(8)).setImage(ManageImage.imageToPImage(pathSelectedImage));
            startDetect();
        }
    }
}
